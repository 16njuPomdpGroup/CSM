...
\begin{abstract}
  Instance-based approaches are effective ways for solving reinforcement
...
\begin{enumerate}
  ...
  \item The agent makes decision and interacts with the environment at time step $t$.
  ...
  action $a$. $U(s')$ is the utility of the state $s'$, calculated as $U(s) = \max_{a \in A} Q(s,a)$.
  $R(s,a)$ and $Pr(s'|s,a)$ are the estimated immediate reward and the transition probability,
  respectively, that drawn from the instance chains:
  ...
  \item Agent perform Kolmogorov-Smirnov test every $n$ instance additions. The expected
  discounted reward of instance $T_i$ is written as $H(T_i)$, and is defined as:
    \begin{equation}
  ...
  \item Agent performs $\epsilon$-greedy method to choose the next action, i.e., with
  probability $1-\epsilon$ the agent chooses $a_{t+1}$ such that
    \begin{equation}
      a_{t+1} = \arg\max_{a \in A} Q(L(T_t), a)
    \end{equation}
  and with probability $\epsilon$ the agent chooses random action in $A$.
  Time step increase to $t+1$ and jumps to step 2.
\end{enumerate}
...
\begin{enumerate}
  ...
  the best policy are shorter than $l$. \label{csm:l}
...
The details of the CSM algorithm is as below (see Algorithm~\ref{alg:CSM}).
...
Our experiment uses a discount factor $\gamma=0.9$, a exploration probability $\epsilon=0.1$.
...
The Experiment result of CSM, USM and random algorithm are shown in Figure 4 to 9.
	...
	\begin{figure}[h]
	  ...
	  \subfigure{
	    ...
	    \centering
	    \includegraphics[width=2in]{tiger_grid_adr_time.png}
	    \end{minipage}
	  }
	  \caption{ADR with time in tiger grid}
	  \label{fig:tiger_grid_adr_time}
	\end{figure}
	...
	\subfigure{
		\begin{minipage}[t]{0.7\linewidth}
			\centering
			\includegraphics[width=2in]{tiger_grid_adr_step.png}
		\end{minipage}
	}
	\caption{ADR with time in tiger grid}
	\label{fig:tiger_grid_adr_step}
	\end{figure}
	...
	\begin{figure}[h]
		...
		\subfigure{
			\begin{minipage}[t]{0.7\linewidth}
				\centering
				\includegraphics[width=2in]{hallway_adr_time.png}
			\end{minipage}
		}
		\caption{ADR with time in hallway}
		\label{fig:hallway_adr_time}
	\end{figure}
	...
	\begin{figure}[h]
		...
		\subfigure{
			\begin{minipage}[t]{0.7\linewidth}
				\centering
				\includegraphics[width=2in]{hallway_adr_step.png}
			\end{minipage}
		}
		\caption{ADR with steps in hallway}
		\label{fig:hallway_adr_step}
	\end{figure}
	...
	\subfigure{
		\begin{minipage}[t]{0.7\linewidth}
			\centering
			\includegraphics[width=2in]{mccallum_adr_time.png}
		\end{minipage}
	}
	\caption{ADR with time in McCallum}
	\label{fig:mccallum_adr_time}
	\end{figure}
	...
	\begin{figure}[h]
		...
		\subfigure{
			\begin{minipage}[t]{0.7\linewidth}
				\centering
				\includegraphics[width=2in]{mccallum_adr_step.png}
			\end{minipage}
		}
		\caption{ADR with steps in McCallum}
		\label{fig:hallway_adr_step}
	\end{figure}
...
In the Tiger Grid problem, the CSM algorithm quickly reached a very high ADR in a short time after the start of the experiment, and as the experiment progressed, the ADR of the CSM was basically stable at around 8. In contrast, the experimental results of the USM algorithm have large fluctuations in the early stage of the experiment, and the experimental results in the latter part of the experiment are basically stable at 3.5, which is much lower than the CSM.


In the Hallway problem, the experimental results of the two algorithms show large fluctuations, which is related to the existence of more hidden states in the Hallway problem. Despite this, CSM's ADR is twice as large as USM.

In the McCallum Maze problem, similar to the performance in the Tiger Grid problem, the CSM algorithm quickly reached a very high ADR in the short time after the start of the experiment, and as the experiment progressed, the ADR of the CSM was basically stable at 6. In contrast, the experimental results of the USM algorithm did not increase significantly in the early stage of the experiment, and only showed a growth trend in the later stage of the experiment. Despite this, USM's final ADR is only about 2, far less than CSM. This shows that in the case of a large problem set, CSM can explore the whole problem more quickly and better than USM, and find the optimal strategy.
