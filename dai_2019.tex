...
The experimental results of CSM, USM and random algorithm are shown in  Table \ref{table:results} and Figure  \ref{fig:tiger_grid_result} to \ref{fig:prim_maze_result}.
%(see Figure~
...
\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{06-13-21-48/tiger_grid_result.png} %1.png是图片文件的相对路径
	\caption{ADR by time(sec) for Tiger-Grid} %caption是图片的标题
	\label{fig:tiger_grid_result} %此处的label相当于一个图片的专属标志，目的是方便上下文的引用
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{06-13-21-48/hallway_result.png} %1.png是图片文件的相对路径
	\caption{ADR by time(sed) for Hallway} %caption是图片的标题
	\label{fig:hallway_result} %此处的label相当于一个图片的专属标志，目的是方便上下文的引用
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{06-13-21-48/mccallum_result.png} %1.png是图片文件的相对路径
	\caption{ADR by time(sec) for McCallum} %caption是图片的标题
	\label{fig:mccallum_result} %此处的label相当于一个图片的专属标志，目的是方便上下文的引用
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.5\textwidth]{06-13-21-48/prim_maze_result.png} %1.png是图片文件的相对路径
	\caption{ADR by time(sec) for Prim Maze} %caption是图片的标题
	\label{fig:prim_maze_result} %此处的label相当于一个图片的专属标志，目的是方便上下文的引用
\end{figure}
