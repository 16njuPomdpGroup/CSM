...
\begin{abstract}
  ...
  heuristic information of environments by blind exploration, for example, the path length
...
policy of USM lack of exploitation of overall information, it may lead to
overfitting.
...
We measure the performance of CSM and see improvements comparing with USM, in Section 4.
We close in Section 5, with a brief summary and possible means of improvement of CSM.
...
In order to deduce state space from those instance chains, USM creates a suffix tree,
...
\begin{enumerate}
  \item The depth of the suffix tree should be constrained. The 3rd Quartile of path lengths
  to goal and the maximum tree depth are denoted as $l_3$ and $d$. Leaves with suffix longer
  than $2l_3$ are considered redundant. During the learning process, $l_3$ decreases and $d$ increases,
  and both become stable. However, the exploitation of immature policy can cause overfitting,
  which makes $d$ grows greater than $2l_3$. So, an alterable guardian depth $l$ is needed.
  On the one hand, $l$ starts with a relatively small value and will gradually increase through
  learning if it is less than $l_3$. On the other hand, $d$ will not grow greater than $2l$.
  In each instance chain, the maximum L1 distance between each instance and the initial instance
  can be calculated.
...
Therefore, we record ($ADR$, time) tuples in the table below (see Table \ref{table:results}), when $ADR$ starts to become relatively stationary, rather than when $ADR$ gets maximum. 
...
\begin{table}[h]
	...
	\begin{tabular}{lcccc}
		\toprule
                  &\multicolumn{2}{c}{$ADR$}      &\multicolumn{2}{c}{time taken} \\ 
